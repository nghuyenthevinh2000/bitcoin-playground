...
func TestLinearTaprootMuSig(t *testing.T) {
	...
	s.validateScript(p2trScript, func(t *testing.T, prevOut *wire.TxOut, tx *wire.MsgTx, sigHashes *txscript.TxSigHashes, idx int) wire.TxWitness {
		...
		sig_3, err := txscript.RawTxInTapscriptSignature(tx, sigHashes, idx, prevOut.Value, p2trScript, tapleaf, txscript.SigHashDefault, pair_3.priv)
		assert.Nil(t, err)

		fmt.Printf("sig_1: %s\n", s.bytesToHexStr(sig_1))
		fmt.Printf("sig_2: %s\n", s.bytesToHexStr(sig_2))
		fmt.Printf("sig_3: %s\n", s.bytesToHexStr(sig_3))
