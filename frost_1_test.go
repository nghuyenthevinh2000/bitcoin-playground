...
func TestFrostSignature1(t *testing.T) {
	...
	for i := int64(0); i < n; i++ {
		participants[i] = testhelper.NewFrostParticipant(&suite, nil, n, threshold, i+1, nil)
	}
	...
	for i := int64(0); i < n; i++ {
		...
		for j := int64(0); j < n; j++ {
			...
			participant.VerifyPublicSecretShares(secret, participant.PolynomialCommitments[participant.Position], uint32(j+1))
		}
	}
}
